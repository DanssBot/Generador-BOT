...
post_reboot() {
  echo 'wget -O /root/install.sh "https://www.dropbox.com/s/h0ep0lpabz5j3c0/Install-Sin-Key.sh"; clear; sleep 2; chmod +x /root/install.sh; /root/install.sh --continue' >>/root/.bashrc
...
install_ADMRufu() {
  ...
  cd /etc
  wget https://www.dropbox.com/s/n8ig3usqst2czwn/ADMRufu.tar.xz >/dev/null 2>&1
